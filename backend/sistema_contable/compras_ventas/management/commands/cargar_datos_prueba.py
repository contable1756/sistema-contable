# compras_ventas/management/commands/cargar_datos_prueba.py
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR("❌ No se encontró un usuario con ID=1. Asegúrate de tener un superusuario.")
            )
            return
        ...
        compras_data = [
            {
                ...
                'base_general': Decimal('500.00'),
                'impuesto_general': Decimal('80.00'),
                'total': Decimal('580.00')
            },
            {
                'numero_factura': 'C-002',
                'subtotal': Decimal('300.00'),
                'base_general': Decimal('300.00'),
                'impuesto_general': Decimal('48.00'),
                'total': Decimal('348.00')
            },
            {
                'numero_factura': 'C-003',
                'subtotal': Decimal('700.00'),
                'base_general': Decimal('700.00'),
                'impuesto_general': Decimal('112.00'),
                'total': Decimal('812.00')
            }
        ]

        for i, data in enumerate(compras_data, start=1):
            compra, created = Compra.objects.get_or_create(
                ...
                defaults={
                    ...
                    'base_imponible_general': data['base_general'],
            ...
            if created:
                ...
                DetalleCompra.objects.create(
                    compra=compra,
                    descripcion=f'Material de oficina - Lote {i}',
                    cantidad=1,
                ...
                self.stdout.write(
                    f"✅ Compra creada: {compra.numero_factura} | Total: {compra.total_compra}"
                )
        ...
        ventas_data = [
            {
                ...
                'impuesto_general': Decimal('160.00'),
                'total': Decimal('1160.00')
            },
            {
                'numero_factura': 'V-002',
                'subtotal': Decimal('600.00'),
                'base_general': Decimal('600.00'),
                'impuesto_general': Decimal('96.00'),
                'total': Decimal('696.00')
            },
            {
                'numero_factura': 'V-003',
                'subtotal': Decimal('900.00'),
                'base_general': Decimal('900.00'),
                'impuesto_general': Decimal('144.00'),
                'total': Decimal('1044.00')
            }
        ]

        for i, data in enumerate(ventas_data, start=1):
            ...
            if created:
                ...
                self.stdout.write(
                    f"✅ Venta creada: {venta.numero_factura} | Total: {venta.total}"
                )
        ...
        self.stdout.write(f"🛒 3 Compras registradas con proveedor: {proveedor}")
        self.stdout.write(f"💰 3 Ventas registradas con cliente: {cliente}")
        self.stdout.write(f"📒 Asientos y movimientos generados correctamente")
        self.stdout.write(f"📊 Verifica en:")
        self.stdout.write(f"   - /api/contabilidad/reportes/libro-diario/")
        self.stdout.write(f"   - /api/contabilidad/reportes/estado-resultados/")
        self.stdout.write(f"   - /api/contabilidad/reportes/balance-general/")
        self.stdout.write("="*60)